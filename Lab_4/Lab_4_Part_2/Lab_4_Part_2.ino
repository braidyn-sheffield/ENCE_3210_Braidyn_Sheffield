...
void loop() 
{
  ...
  switch(currentState){
    case SOLARPANELCHARGE: // State where the solar panel is supplying power to the microprocessor
      ...
      {
        digitalWrite(LED, HIGH); // Turn on the LED indicating that the solar panel is charging the battery
      }
      else
      {
        digitalWrite(LED, LOW); // Turn off the LED indicating that the battery is not charging
      }
